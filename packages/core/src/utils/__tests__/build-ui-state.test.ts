...
import type { Branch, Commit, Repo, WorkingTreeStatus } from '@teapot/contract';
...
describe('buildUiState', () => {
  ...
  it('creates a trunk stack with spinoffs and branch annotations', () => {
    ...
    expect(featureStack.commits.map((commit) => commit.sha)).toEqual([featureBase.sha, featureTip.sha]);
  ...
  it('orders spinoff stacks and their commits deterministically by timestamp', () => {
    ...
    expect(featureAStack.commits.map((commit) => commit.sha)).toEqual([featureA.sha, featureATip.sha]);
    expect(featureBStack.commits.map((commit) => commit.sha)).toEqual([featureB.sha, featureBtip.sha]);
  });
  ...
  it('continues non-trunk stacks on the earliest child and spawns additional spinoffs', () => {
    ...
    expect(featureStack.commits.map((commit) => commit.sha)).toEqual([featureBase.sha, featureEarly.sha]);
  ...
  it('prevents duplicate membership when spinoffs converge to the same commit', () => {
    ...
    expect(featureStack.commits.map((commit) => commit.sha)).toEqual([featureBase.sha, featureA.sha]);
    const nestedFromBase = featureStack.commits[0]?.spinoffs[0];
    if (!nestedFromBase) {
      throw new Error('expected nested spinoff');
    }
    expect(nestedFromBase.commits.map((commit) => commit.sha)).toEqual([featureB.sha, sharedTip.sha]);
  });
  ...
  it('ignores unknown child shas when building spinoffs', () => {
    ...
    expect(featureStack.commits.map((commit) => commit.sha)).toEqual([featureBase.sha, featureChild.sha]);
  });
  ...
  it('creates deeply nested spinoffs while retaining branch annotations', () => {
    ...
    expect(feature1Stack.commits.map((commit) => commit.sha)).toEqual([feature1Base.sha, feature1Tip.sha]);
    ...
    expect(feature2Stack.commits.map((commit) => commit.sha)).toEqual([feature2Base.sha, feature2Tip.sha]);
