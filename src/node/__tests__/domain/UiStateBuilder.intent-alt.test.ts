...
const buildFullUiState = UiStateBuilder.buildFullUiState
...
describe('buildUiState (alt projection scenario)', () => {
  it('keeps baseline stack layout before the rebase intent', () => {
    ...
    if (!trunkTip) throw new Error('expected trunk tip')

    expect(trunkTip.spinoffs).toHaveLength(3)
    const thirdSpinoff = trunkTip.spinoffs[2]
    expect(thirdSpinoff?.commits.map((commit) => commit.sha)).toEqual([COMMITS.stack985])

    const continuation = trunkTip.spinoffs[0]?.commits ?? []
    const threeCommit = continuation.find((commit) => commit.sha === COMMITS.stack3e)
    expect(threeCommit?.spinoffs).toEqual([])
  })

  it('shows the rebased branch as a new spinoff under the feature chain', () => {
    const repo = createSampleRepo()
    const intent = createDemoRebaseIntent()
    const uiState = buildFullUiState(repo, { rebaseIntent: intent })
    const projected = uiState.projectedStack
    expect(projected).not.toBeNull()
    if (!projected) throw new Error('expected projected stack')

    const trunkTip = projected.commits.at(-1)
    if (!trunkTip) throw new Error('expected trunk tip')

    const featureContinuation = trunkTip.spinoffs[0]
    expect(featureContinuation?.commits.map((commit) => commit.sha)).toEqual([
      COMMITS.stack827,
      COMMITS.stack3e
    ])

    const projectedSpinoff = trunkTip.spinoffs.find(
      (stack) => stack.commits[0]?.sha === COMMITS.stack985
    )
    if (!projectedSpinoff) {
      throw new Error('expected projected spinoff')
    }
    expect(projectedSpinoff.commits.map((commit) => commit.sha)).toEqual([
      COMMITS.stack985,
      COMMITS.stack234,
      COMMITS.stackBf5,
      COMMITS.stack799,
    ...
    const projected985 = projectedSpinoff.commits[0]
    if (!projected985) {
      throw new Error('expected projected 985')
    }
    const newBase = featureContinuation?.commits[1]
    if (!newBase) {
      throw new Error('expected base commit for projection')
    }
    expect(projected985.timestampMs).toBeGreaterThan(newBase.timestampMs)
  })
})
...
function createDemoRebaseIntent(): RebaseIntent {
  return {
    id: 'demo-alt',
    createdAtMs: Date.parse('2025-11-16T16:23:06.383Z'),
    targets: [
      {
