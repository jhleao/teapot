...
const getSquashPreview: IpcHandlerOf<'getSquashPreview'> = async (
  ...
  const workingPath = resolveWorkingPath(repoPath)
  return SquashOperation.preview(workingPath, commitSha)
}

const squashIntoParent: IpcHandlerOf<'squashIntoParent'> = async (
  _event,
  { repoPath, commitSha, commitMessage, branchResolution }
) => {
  const workingPath = resolveWorkingPath(repoPath)
  return SquashOperation.execute(workingPath, commitSha, { commitMessage, branchResolution })
}
