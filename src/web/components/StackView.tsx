...
export const CommitView = memo(function CommitView({
  ...
  const firstBranchfulCommit = stack.commits.find((c) => c.branches.length > 0)
  const isFirstBranchfulCommit = firstBranchfulCommit?.sha === data.sha
    ...
    <div className={cn('w-full pl-2 whitespace-nowrap')}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {data.branches.length > 0 && <MultiBranchBadge branches={data.branches} stack={stack} />}
          ...
          {data.rebaseStatus !== 'prompting' && data.rebaseStatus !== 'queued' && (
            <>
              <GitForgeSection
                ...
                canRebaseToTrunk={stack.canRebaseToTrunk && isFirstBranchfulCommit}
              />
