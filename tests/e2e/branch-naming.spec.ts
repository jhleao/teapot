...
testWithRepo.describe('Branch Naming', () => {
  ...
  testWithRepo.describe('Branches with slashes (nested paths)', () => {
    ...
    testWithRepo('handles multiple slashes (deeply nested)', async ({ page, gitRepo }) => {
      ...
      gitRepo.commitFile('src/settings.ts', 'export const settings = {}', 'Add user profile settings')
  ...
  testWithRepo.describe('Unicode in branch names', () => {
    // Skip: Git allows these but they can cause issues with some tools
    testWithRepo.skip('handles emoji in branch names', async ({ page, gitRepo }) => {
      await expect(page.getByTestId('app-container')).toBeVisible()
      ...
      await expect(page.getByTestId('stack-view').first()).toBeVisible({ timeout: 15000 })
      // TODO: Check if branch name renders correctly with emoji
    })

    // Skip: International characters in branch names
    testWithRepo.skip('handles non-ASCII characters in branch names', async ({ page, gitRepo }) => {
      await expect(page.getByTestId('app-container')).toBeVisible()
      ...
      await expect(page.getByTestId('stack-view').first()).toBeVisible({ timeout: 15000 })
      // TODO: Verify Unicode renders correctly
    })
  })
