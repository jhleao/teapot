...
testWithRepo.describe('Complex Repository Structures', () => {
  testWithRepo.describe('Multi-branch at different depths', () => {
    testWithRepo('can load repo with 5+ branches at different depths', async ({ page, gitRepo }) => {
      ...
      gitRepo.commitFile('src/api/users.ts', 'export function getUsers() {}', 'Add users endpoint')
    ...
    testWithRepo('can load repo with deeply nested stacked branches (3+ levels)', async ({ page, gitRepo }) => {
      await expect(page.getByTestId('app-container')).toBeVisible()
  ...
  testWithRepo.describe('Merge commits', () => {
    testWithRepo('can load repo with merge commits in history', async ({ page, gitRepo }) => {
      ...
      gitRepo.commitFile('src/feature.ts', 'export function feature() { return 1 }', 'Improve feature')
