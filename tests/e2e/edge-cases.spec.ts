...
testWithRepo.describe('Edge Cases', () => {
  testWithRepo.describe('Dirty working tree', () => {
    ...
    testWithRepo(
      'handles repo with both staged and unstaged changes',
      async ({ page, gitRepo }) => {
        await expect(page.getByTestId('app-container')).toBeVisible()
...
testWithEdgeCases.describe('Empty Repository', () => {
  testWithEdgeCases('handles empty repo with no commits', async ({ page, emptyRepo }) => {
    ...
    // App should handle empty repo - might show error state or loading
    // The key is it shouldn't crash - app container should remain visible
  ...
  testWithEdgeCases('empty repo with staged but uncommitted files', async ({ page, emptyRepo }) => {
    ...
    // App should handle this gracefully - just verify it doesn't crash
    await page.waitForTimeout(3000)
  ...
  testWithEdgeCases('error state shows reload button', async ({ page, emptyRepo }) => {
    await expect(page.getByTestId('app-container')).toBeVisible()

    // Add the empty repo (likely to cause error)
    await addRepoToApp(page, emptyRepo.repoPath)

    // Wait for potential error state
    await page.waitForTimeout(2000)

    // If error state is shown, verify reload button is present
    ...
    if (hasErrorState) {
      await expect(page.getByTestId('error-reload-button')).toBeVisible()
    }
  })
})
