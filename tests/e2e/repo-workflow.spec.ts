...
// TODO: These tests need fixes - branch/commit UI selectors don't match current app
testWithRepo.describe.skip('Repository with Branches', () => {
  testWithRepo('shows feature branch in stack view', async ({ page, gitRepo }) => {
    ...
    // Should see the branch name somewhere in the UI (it's in a badge)
    await expect(page.locator('text=feature/login')).toBeVisible({ timeout: 5000 })
  })
  ...
  testWithRepo('shows stacked branches correctly', async ({ page, gitRepo }) => {
    ...
    // Both branches should be visible (as badges or text)
    await expect(page.locator('text=feature/auth-ui')).toBeVisible({ timeout: 5000 })
    await expect(page.locator('text=feature/auth').first()).toBeVisible({ timeout: 5000 })
  })
})
